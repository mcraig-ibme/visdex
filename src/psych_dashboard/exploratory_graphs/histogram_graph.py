...
@app.callback(
    ...
    [State({'type': 'histogram-' + component['id'], 'index': MATCH}, prop)
     for component in all_components['histogram'] for prop in component]
)
def update_histogram_components(df_loaded, style_dict, *args):
    ...
    return update_graph_components('histogram', all_components['histogram'], dd_options, args)


@app.callback(
    Output({'type': 'gen-histogram-graph', 'index': MATCH}, "figure"),
    [*(Input({'type': 'histogram-' + component['id'], 'index': MATCH}, "value") for component in all_components['histogram'])],
)
